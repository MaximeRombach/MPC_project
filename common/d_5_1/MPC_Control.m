...
%       mpc.ctrl_opt = mpc.setup_controller();
    ...
    function u = get_u(mpc, x, ref)
      ...
      mpc.ctrl_opt=mpc.setup_controller(ref_x,ref_u);
      % Compute the control action
      if length(struct(mpc.ctrl_opt).diminOrig) == 2
        if length(x) == 3
          d_est = x(end);
          x = x(1:end-1);
        else
        ...
        [u, isfeasible] = mpc.ctrl_opt({x-ref_x, d_est});        
      else
        [u, isfeasible] = mpc.ctrl_opt(x-ref_x);
      end
